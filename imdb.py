...
def parse_imdb_data(imdb_data: dict, tmdb_data: dict = None) -> dict:
    """
...
def update_series_metadata(series_id: int, metadata: dict, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        for field in ['imdb_id', 'tmdb_id', 'name', 'year', 'summary', 'genres',
                      'language', 'rating', 'trailer_key', 'poster_url', 'backdrop_url']:
...
def process_series(series_id: int = None, limit: int = 10, dry_run: bool = False) -> int:
    ...
    try:
        ...
        for series in series_list:
            ...
            # Step 4: Get trailer if we have TMDB ID
            ...
            metadata = parse_imdb_data(imdb_data, tmdb_data)
